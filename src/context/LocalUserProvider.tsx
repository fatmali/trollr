...
interface LocalUserContextType {
  ...
  addReward: (type: Reward['type'], content: string, associatedIds?: { taskId?: string, achievementId?: string }) => void;
  rewards: Reward[];
...
const defaultUserPreferences: User['preferences'] = {
  ...
  lofiEnabled: false, // Lofi music disabled by default
  lofiVolume: 50, // Medium volume by default
  lofiStation: 'lofi-radio', // Default station
};
...
export function LocalUserProvider({ children }: { children: React.ReactNode }) {
  ...
  const [preferences, setPreferences] = useState<User['preferences']>(defaultUserPreferences);
  ...
  useEffect(() => {
    if (userId) {
      const calculateProductivityScore = () => {
        ...
        const completionRatio = totalTasks > 0 ? (stats.tasksCompleted / totalTasks) : 0.5;
        
        // Base score from pomodoro success ratio
        const totalPomodoros = stats.pomodorosCompleted + stats.pomodorosAbandoned;
        const pomodoroRatio = totalPomodoros > 0 ? (stats.pomodorosCompleted / totalPomodoros) : 0.5;
  ...
  }, [stats.tasksCompleted, stats.tasksOverdue, stats.pomodorosCompleted, stats.pomodorosAbandoned, stats.currentStreak, userId]);
  ...
  const unlockAchievement = (achievementId: string) => {
    ...
    localStorage.setItem('troll_achievements', JSON.stringify(updatedAchievements));
    
    // Add a reward for the achievement
    addReward(
      'badge',
      `Achievement Unlocked: ${achievements.find(a => a.id === achievementId)?.name}`,
      { achievementId }
    );
  };
  ...
  const addReward = (
    ...
    const newReward: Reward = {
      ...
      ...(associatedIds?.achievementId && { associatedAchievementId: associatedIds.achievementId })
    };
  ...
  useEffect(() => {
    ...
    if (savedPreferences) {
      try {
        setPreferences({ ...defaultUserPreferences, ...JSON.parse(savedPreferences) });
      } catch (error) {
    ...
    if (savedAchievements) {
      ...
      setAchievements(defaultAchievements);
      localStorage.setItem('troll_achievements', JSON.stringify(defaultAchievements));
    }
    ...
    const checkInitialStreak = () => {
      const today = new Date().toISOString().split('T')[0];
      const lastActive = savedStats ? JSON.parse(savedStats).lastActiveDate : null;
  ...
  const updatePreferences = (newPreferences: Partial<User['preferences']>) => {
    ...
    localStorage.setItem('troll_preferences', JSON.stringify(updatedPreferences));
